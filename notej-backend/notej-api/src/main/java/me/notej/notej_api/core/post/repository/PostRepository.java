...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("SELECT p FROM Post p WHERE p.category.id = :categoryId AND p.updated_at <= :updated_at ORDER BY p.updated_at DESC")
    List<Post> findByCategoryIdAndCreatedAtLessThanEqual(
